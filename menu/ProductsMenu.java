...
public final class ProductsMenu {
    ...
    public ProductsMenu(ProductService products) {
    ...
    private void takeLoan() throws SQLException {
        ...
        Loan loan = products.openLoan(number, ConsoleIO.readDouble(Strings.PROMPT_ORIGINAL_AMOUNT),
    ...
    private void showLoans() throws SQLException {
        ConsoleIO.printAll(
                products.getLoansForAccount(ConsoleIO.readInt(Strings.PROMPT_ACCOUNT_NUMBER)),
                Strings.NONE_FOUND);
    }
    ...
    private void takeMortgage() throws SQLException {
        int number = ConsoleIO.readInt(Strings.PROMPT_ACCOUNT_NUMBER);
        Mortgage mortgage = products.openMortgage(number,
    ...
    private void showMortgages() throws SQLException {
        ConsoleIO.printAll(
                products.getMortgagesForAccount(ConsoleIO.readInt(Strings.PROMPT_ACCOUNT_NUMBER)),
                Strings.NONE_FOUND);
    }
    ...
    private void issueCard() throws SQLException {
        ...
        Card card = switch (ConsoleIO.readInt(Strings.PROMPT_CHOICE)) {
            case 1 -> products.issueCard(number, Card.CardType.DEBIT, 0);
            case 2 -> products.issueCard(number, Card.CardType.CREDIT,
